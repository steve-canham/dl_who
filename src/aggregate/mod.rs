...
*/
    ...
    data_access::set_up_sec_id_tables(pool).await?;
    for entry in &tables {
        let (tr, oth) = data_access::process_sec_ids(entry, pool).await?;
        tr_ids_total += tr;
        oth_ids_total += oth;
    }
    ...
    data_access::set_up_ftw_schema("cxt.locs", pool).await?;
  ...
  //  data_access::insert_grid_status_year_data(pool).await?;
    ...
    data_access::drop_ftw_schema("ctx_locs", pool).await?;
